...
```{r}
names_2014 <- names(inscritos_2014)


```


*Observación:* Si bien el resultado muestra que existen variables que no son las mismas, pero el valor que contienen corresponde al mismo.


## Combinar dataset y generar un archivo .csv
```{r}
# Combinar todos los datasets en un solo data frame
library(dplyr)
inscritos <- bind_rows(lista_inscritos)

nombres_columnas <- colnames(inscritos)
nombres_columnas[length(nombres_columnas)] <- "cantidad_inscritos"
colnames(inscritos) <- nombres_columnas
